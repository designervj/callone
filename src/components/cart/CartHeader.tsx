...
export const CartHeader: React.FC<CartHeaderProps> = ({
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {/* Retailer Section */}
      <div className={clsx(
        "group relative z-10 rounded-[24px] border border-border bg-surface p-8 shadow-[0_10px_30px_rgba(0,0,0,0.08)] backdrop-blur-2xl transition-all duration-300 hover:-translate-y-0.5 focus-within:z-50",
        selectedRetailer ? "border-primary/20" : "border-border"
      )}>
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={clsx(
              "flex h-8 w-8 items-center justify-center rounded-xl transition-colors duration-500",
              selectedRetailer ? "bg-primary text-surface" : "bg-foreground/[0.04] text-foreground/30"
            )}>
          ...
          {selectedRetailer && !isEditingRetailer && (
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-lg bg-foreground/[0.04] text-foreground/30 transition-all hover:bg-primary hover:text-surface"
            >
              <Edit2 size={14} />
            </button>
          )}
        ...
        {!selectedRetailer || isEditingRetailer ? (
          <div className="space-y-4">
            ...
            />
            {!selectedRetailer && (
               <div className="flex items-center gap-2 rounded-lg border border-border bg-foreground/[0.03] px-3 py-2 text-foreground/54">
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              <div className="flex items-center gap-2 rounded-lg border border-border bg-foreground/[0.03] px-3 py-1.5">
                ...
                <Check size={10} className="text-primary/70" strokeWidth={4} />
              </div>
              <div className="flex items-center gap-2 rounded-lg border border-border bg-foreground/[0.03] px-3 py-1.5">
      ...
      <div className={clsx(
        "group relative z-10 rounded-[24px] border border-border bg-surface p-8 shadow-[0_10px_30px_rgba(0,0,0,0.08)] backdrop-blur-2xl transition-all duration-300 hover:-translate-y-0.5 focus-within:z-50",
        selectedManager ? "border-primary/20" : "border-border"
      )}>
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={clsx(
              "flex h-8 w-8 items-center justify-center rounded-xl transition-colors duration-500",
              selectedManager ? "bg-primary text-surface" : "bg-foreground/[0.04] text-foreground/30"
            )}>
        ...
        {!selectedManager || isEditingManager ? (
          ...
          >
            ...
            <div className="flex items-center gap-2">
               <div className="h-1.5 w-1.5 rounded-full bg-primary/70 shadow-[0_0_10px_rgba(var(--primary-rgb),0.25)]" />
      ...
      <div className={clsx(
        "group relative z-10 rounded-[24px] border border-border bg-surface p-8 shadow-[0_10px_30px_rgba(0,0,0,0.08)] backdrop-blur-2xl transition-all duration-300 hover:-translate-y-0.5 focus-within:z-50",
        selectedSalesRep ? "border-primary/20" : "border-border"
      )}>
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={clsx(
              "flex h-8 w-8 items-center justify-center rounded-xl transition-colors duration-500",
              selectedSalesRep ? "bg-primary text-surface" : "bg-foreground/[0.04] text-foreground/30"
            )}>
