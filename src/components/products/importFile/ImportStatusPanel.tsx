...
export default function ImportStatusPanel({
  ...
  return (
    <div className="space-y-4 rounded-[20px] border border-white/8 bg-[color:var(--surface)] p-5 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
      <div className="flex items-start gap-4">
        <div className="rounded-xl border border-white/8 bg-white/[0.04] p-3 text-white">
        ...
        <button
          onClick={onClear}
          className="p-1 text-foreground/40 transition-colors hover:text-white disabled:opacity-40"
      ...
      {(status === 'uploading' || progress > 0) && (
        <div>
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/10">
            <div className="h-full bg-white transition-[width] duration-300 ease-out shadow-[0_0_12px_rgba(255,255,255,0.35)]" style={{ width: `${progress}%` }} />
          </div>
          ...
          <div className="mt-3 max-h-48 space-y-2 overflow-auto pr-2">
            {visibleErrors.map((issue, index) => (
              <div
                ...
                className="rounded-2xl border border-white/8 bg-[color:var(--surface)] px-3 py-2 text-xs text-foreground/66"
              >
